...
// Flag to track if we're using the mock service - set to true by default
let usingMockService = true;

// Function to check if Supabase is available - always returns false to force mock service
const checkSupabaseAvailability = async (): Promise<boolean> => {
  // Disabled Supabase authentication - always use mock service
  console.log('Supabase authentication disabled. Using local mock authentication.');
